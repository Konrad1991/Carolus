...
void delete_field(ObjectArray *objects, Map *map, GameState *game_state, int mansus_idx, bool icon_hovered) {
  if (mansus_idx < 0 || icon_hovered || !IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) return;
  Mansus *m = &game_state->mansen.data[mansus_idx];
  if (m->fields.count == 0) return;
  Field *f = &m->fields.data[0];
  ...
  field_array_remove_swap(&m->fields, 0);
}

static int find_mansus_at(GameState *game_state, int x, int y) {
...
void delete_object(ObjectArray *objects, Map *map,
  ...
  if (o->kind == OBJECT_MANSUS) {
    ...
    if (mansus_idx >= 0) {
      Mansus *m = &game_state->mansen.data[mansus_idx];
      if (m->living_house != 0 || m->barn != 0 || m->fields.count > 0) return;
...
void build_object(ObjectArray *objects, const BuildPreview *preview,
  ...
  if (mode_state.current == BUILD_OAK) {
    ...
    scatter_boundary_stones(objects, map, texture_state, tx, ty, current_obj.footprint_w, current_obj.footprint_h);
  }
...
void update_field_action(FieldAssignState *field_state, TileState tile_state, ModeState *mode_state,
  ...
  for (int mi = 0; mi < game_state->mansen.count; mi++) {
    Mansus *m = &game_state->mansen.data[mi];
    bool eligible = m->fields.count == 0;
    Color Yellow = (Color){230, 200, 60, 140};
    Color Red = (Color){200, 0, 0, 110};
  ...
  for (int fy = new_field.corners_field[0][1]; fy <= new_field.corners_field[1][1]; fy++) {
    for (int fx = new_field.corners_field[0][0]; fx <= new_field.corners_field[1][0]; fx++) {
      Tile *ft = map_tile(map, fx, fy);
      if (ft) ft->type = TILE_SOIL;
    }
  }
